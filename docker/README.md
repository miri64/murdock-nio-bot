...
The docker image will run murdock-nio-bot with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/murdock-nio-bot?sslmode=disable"
  ```
...
```
docker-compose up murdock-nio-bot
```
...
```
docker-compose up -d murdock-nio-bot
```

(Logs can later be accessed with the `docker logs` command).

This will use the `latest` tag from
[Docker Hub](https://hub.docker.com/somebody/murdock-nio-bot).
...
```
docker-compose pull murdock-nio-bot
```
...
[murdock-nio-bot.service](murdock-nio-bot.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/murdock-nio-bot.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start murdock-nio-bot
```

and stop it with:

```
systemctl stop murdock-nio-bot
```

To run the bot on system startup:

```
systemctl enable murdock-nio-bot
```
...
```
docker build -t somebody/murdock-nio-bot:latest -f docker/Dockerfile .
```
